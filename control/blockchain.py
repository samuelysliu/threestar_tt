...
def thunderCore():
    web3 = Web3(Web3.HTTPProvider(os.getenv("httpProvider"),
...
def bsc():
    web3 = Web3(Web3.HTTPProvider(os.getenv("httpProvider_bsc"),
                request_kwargs={'timeout': 60}))
    web3.middleware_onion.inject(geth_poa_middleware, layer=0)
...
def getThreeStarContract(web3):
    ...
    threeStarContract = web3.eth.contract(
        address=threeStarContractAddress, abi=threeStarABI)
...
def getThreeStarContract_bsc(web3):
    ...
    threeStarContractAddress = os.getenv(
        "REACT_APP_ThreeStarContractAddress_bsc")
    threeStarContract = web3.eth.contract(
        address=threeStarContractAddress, abi=threeStarABI)
...
def getStakeContract(web3):
    ...
    stakeContract = web3.eth.contract(
        address=stakeContractAddress, abi=stakeABI)
...
def getPlayerAmount(web3, contract, playerAddress):
    playerAmount = web3.fromWei(
        contract.functions.playerInfo(playerAddress).call, 'ether')
    return playerAmount
...
def getAPR(web3, dividends, stakeContract):
    try:
        totalSupply = web3.fromWei(
            stakeContract.functions.totalSupply().call(), 'ether')
...
def getTotalStake(web3, TSTokenContract, stakeContractAddress):
    try:
        totalStake = web3.fromWei(TSTokenContract.functions.balanceOf(
            stakeContractAddress).call(), 'ether')
        return round(totalStake, 5)
    except:
        return "failed"
